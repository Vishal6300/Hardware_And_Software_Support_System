...
//  This class implements the EngineerDao interface that has methods of Engineer Panel
	...
	public String updateComplaintStatus(int complaintId, String newStatus) throws ComplaintException {
		...
		try (Connection conn = DBUtil.provideConnection()) {
			...
			if (newStatus.equals("Resolved")) {
				PreparedStatement ps = conn
						.prepareStatement("update complaints set status=?, dateResolved=? where complaintId=?");
				...
				PreparedStatement ps = conn
						.prepareStatement("update complaints set status=?,dateResolved=null where complaintId=?");
			...
			if (x > 0) {
				...
				throw new ComplaintException(
						"Could not find complaint with complaintid " + complaintId + " Please try again.");
			}
